...
type Props = {
  ...
  credentials: whatsAppLiteCredentials['data'] &
    Pick<whatsAppLiteCredentials, 'id'>
  contact: NonNullable<SessionState['whatsappLite']>['contact']
}

export const startWhatsAppLiteSession = async ({
  incomingMessage,
  workspaceId,
  credentials,
  contact,
...
> => {
  ...
  const botsWithWhatsAppEnabled = publicTypebotsWithWhatsAppEnabled.filter(
    (publicTypebot) =>
      ...
      publicTypebot.settings.whatsappLite?.isEnabled
  )

  const publicTypebotWithMatchedCondition = botsWithWhatsAppEnabled.find(
    (publicTypebot) =>
      (publicTypebot.settings.whatsappLite?.startCondition?.comparisons
        .length ?? 0) > 0 &&
      messageMatchStartCondition(
        incomingMessage ?? '',
        publicTypebot.settings.whatsappLite?.startCondition
      )
  )
  ...
  const publicTypebot =
    ...
    botsWithWhatsAppEnabled.find(
      (publicTypebot) => !publicTypebot.settings.whatsappLite?.startCondition
    )
  ...
  const sessionExpiryTimeoutHours =
    publicTypebot.settings.whatsappLite?.sessionExpiryTimeout ??
    defaultSessionExpiryTimeout
