...
import { HttpMethod } from '@typebot.io/schemas/features/blocks/integrations/whatsapp/constants'

test.describe('Builder', () => {
  test('easy configuration should work', async ({ page }) => {
    const typebotId = createId()
    await importTypebotInDatabase(
      getTestAsset('typebots/integrations/easyConfigWhatsapp.json'),
      {
        id: typebotId,
      }
    )
    await createWhatsapp(typebotId, { method: HttpMethod.POST })
    await page.goto(`/typebots/${typebotId}/edit`)
    await page.click('text=Configure...')
    await page.fill(
      'input[placeholder="Paste whatsapp URL..."]',
      `${env.NEXTAUTH_URL}/api/mock/whatsapp-easy-config`
    )
  ...
  test('its configuration should work', async ({ page }) => {
    const typebotId = createId()
    await importTypebotInDatabase(
      getTestAsset('typebots/integrations/whatsapp.json'),
      {
        id: typebotId,
      }
    )
    await createWhatsapp(typebotId)

    await page.goto(`/typebots/${typebotId}/edit`)
    await page.click('text=Configure...')
    await page.fill(
      'input[placeholder="Paste whatsapp URL..."]',
      `${env.NEXTAUTH_URL}/api/mock/whatsapp`
    )
...
test.describe('API', () => {
  ...
  test('can get whatsapp blocks', async ({ request }) => {
    const response = await request.get(
      `/api/v1/typebots/${typebotId}/whatsappBlocks`,
      {
        headers: { Authorization: `Bearer ${apiToken}` },
      }
    )
    const { whatsappBlocks } = await response.json()
    expect(whatsappBlocks).toHaveLength(1)
    expect(whatsappBlocks[0]).toEqual({
      id: 'whatsappBlock',
      label: 'Whatsapp > whatsappBlock',
      type: 'Whatsapp',
    })
  })

  test('can subscribe whatsapp', async ({ request }) => {
    const url = 'https://test.com'
    const response = await request.post(
      `/api/v1/typebots/${typebotId}/whatsappBlocks/whatsappBlock/subscribe`,
      {
        headers: {
  ...
  test('can unsubscribe whatsapp', async ({ request }) => {
    const response = await request.post(
      `/api/v1/typebots/${typebotId}/whatsappBlocks/whatsappBlock/unsubscribe`,
      {
        headers: { Authorization: `Bearer ${apiToken}` },
      }
    )
  ...
  test('can get a sample result', async ({ request }) => {
    const response = await request.get(
      `/api/v1/typebots/${typebotId}/whatsappBlocks/whatsappBlock/getResultExample`,
      {
        headers: { Authorization: `Bearer ${apiToken}` },
      }
    )
